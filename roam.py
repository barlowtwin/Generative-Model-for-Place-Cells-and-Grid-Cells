...
def random_walk_wo_wall(model):
        ...
        for t in range(model.total_dim):
            if t == 0:
                ...
                if new_continue_action_flag:
                    ...
                    action_duriation = np.random.poisson(2, 1) # (lambda,size)

                if action_duriation > 0:
                    if not need_to_stop:
                        if action_random_selection == 0:
                            if position[index_sample, 1, t - 1] == 8: # cannot move down anymore
                                need_to_stop = True
                                position[index_sample, :, t] = position[index_sample, :, t - 1] # go back to previous position
                                action_selection[index_sample, t - 1] = 4
                            else:
                                position[index_sample, :, t] = position[index_sample, :, t - 1] + np.array([0, 1]) # move down
                                action_selection[index_sample, t - 1] = action_random_selection

                        elif action_random_selection == 1:
                            if position[index_sample, 1, t - 1] == 0: # cannot move left anymore
                                need_to_stop = True
                                position[index_sample, :, t] = position[index_sample, :, t - 1]
                                action_selection[index_sample, t - 1] = 4
                            else:
                                position[index_sample, :, t] = position[index_sample, :, t - 1] + np.array([0, -1]) # move up
                                action_selection[index_sample, t - 1] = action_random_selection

                        elif action_random_selection == 2: # cannot move left anymore
                        ...
                        elif action_random_selection == 3: # cannot move right anymore
    ...
    # returns one hot vector of actions
    # returns position
    # returns action selection
